...
def load_arguments(self, _):
    ...
    with self.argument_context('acr cache') as c:
        ...
        c.argument('sync', arg_type=get_enum_type(['ActiveSync', 'PassiveSync']), help='PassiveSync indicates the rule behaves as a pull-through cache while ActiveSync indicates the rule actively synchronizes artifacts from the upstream repository. Default is PassiveSync.')
        ...
        c.argument('starts_with', help='[DEPRECATED] Only sync the tags that start with the specified string. Use --tag-starts-with instead.', deprecate_info=c.deprecate(redirect='--tag-starts-with'))
        c.argument('tag_starts_with', options_list=['--tag-starts-with'], help='Only sync the tags that start with the specified string.')
        c.argument('ends_with', help='[DEPRECATED] Only sync the tags that end with the specified string. Use --tag-ends-with instead.', deprecate_info=c.deprecate(redirect='--tag-ends-with'))
        c.argument('tag_ends_with', options_list=['--tag-ends-with'], help='Only sync the tags that end with the specified string.')
        c.argument('contains', help='[DEPRECATED] Only sync the tags that contain the specified string. Use --tag-contains instead.', deprecate_info=c.deprecate(redirect='--tag-contains'))
        c.argument('tag_contains', options_list=['--tag-contains'], help='Only sync the tags that contain the specified string.')
        c.argument('tag', help='[DEPRECATED] Only sync the artifacts with this exact tag name. This is an exact match filter. Use --tag-equals instead.', deprecate_info=c.deprecate(redirect='--tag-equals'))
        c.argument('tag_equals', options_list=['--tag-equals'], help='Only sync the artifacts with this exact tag name. This is an exact match filter.')
        ...
        c.argument('include_artifact_types', help='A list of artifact types to include in the sync. e.g. application/vnd.docker.distribution.manifest.v2+json')
        c.argument('exclude_artifact_types', help='A list of artifact types to exclude from the sync. e.g. application/vnd.docker.distribution.manifest.v2+json')
        c.argument('include_image_types', help='A list of image types to include in the sync. e.g. application/vnd.docker.distribution.manifest.v2+json')
